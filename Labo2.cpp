...
***************************************************************************************/
int qspartition (vector<int>& v, int inizio, int fine) {
  int pivot = inizio;
  scambia(v, inizio, pivot);
  int i= inizio+1;

  for (int j = inizio+1; j<=fine; j++) {
    if (v[j] < v[inizio]) {             //confronto con il pivot e se è più piccolo...
      scambia (v, i, j);                //scambio i e j e...
      i++;                              //aumento i
      }
    j++;                                //aumento j a prescindere
  }

  scambia (v, inizio, i-1)              //metto il pivot nella sua posizione giusta, ovvero i-1
}

void qs (vector<int>& v, int inizio, int fine) {
  int pivot_index = qspartition (v, inizio, fine);
  qs (v, inizio, pivot_index-1);        //chiamo qs sulla prima metà
  qs (v, pivot_index+1, fine);          //chiamo qs sullla seconda metà
}
